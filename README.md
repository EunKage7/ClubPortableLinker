...
### 2026-06-05 (UX-доработки интерфейса)
- **Цветной журнал**: ошибки — красным, успех — зелёным, предупреждения — янтарным, прогресс — приглушённым; ошибки видно без вычитывания. Кнопки у журнала: **⤢** большое окно, **⧉** скопировать всё, **✕** очистить (файл лога сохраняется).
- **Живая пилюля «игровой диск»** в шапке: какой диск выбран под пакеты/игры и сколько свободно; диск пропал (его физически переключают) — красный сигнал. Обновляется сама каждые 15 с.
- **Индикатор «⏳ выполняется…»**: пилюля в шапке + суффикс в заголовке окна (видно из панели задач) на время любой фоновой операции.
- **Фильтр в каталоге пакетов**: строка поиска по имени/пути — на сервере с десятками пакетов быстрее, чем листать.
- **Кнопка «?»** в шапке — открывает GUIDE.md (теперь едет рядом с exe).
- **Окно запоминает размер/позицию** между запусками (восстанавливается только на видимый монитор).
